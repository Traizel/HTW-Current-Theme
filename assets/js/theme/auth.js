...
export default class Auth extends PageManager {
	...
	registerCreateAccountValidator($createAccountForm) {
		...
		const email2Selector = `${this.formCreateSelector} [data-label="Confirm Email"]`;
		const $email2Element = $(email2Selector);
		...
		if ($emailElement && $email2Element) {
			createAccountValidator.remove(emailSelector);
			createAccountValidator.remove(email2Selector);
			Validators.setConfirmEmailValidation(
				createAccountValidator,
				emailSelector,
				email2Selector,
				this.validationDictionary.valid_email
			);
		}
	...
	shuffleCreateAccountFields(formCreateSelector) {
		const emailField = document.querySelector('[data-type="EmailAddress"]');
		const confirmEmailField = Array.prototype.slice.call(document.querySelectorAll(`${formCreateSelector} .form-label`)).filter(label => {
			return label.innerText.indexOf('Confirm Email') > -1;
		}).reduce((acc, label) => {
			const formFieldId = label.htmlFor.match(/FormField_\d+/);
		...
		if (emailField && confirmEmailField) {
			emailField.insertAdjacentElement('afterend', confirmEmailField);
		}
	}
	/**
	...
	onReady() {
		...
		if ($createAccountForm.length) {
			this.shuffleCreateAccountFields(this.formCreateSelector);
